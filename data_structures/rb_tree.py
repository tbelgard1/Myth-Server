"""
Red-black tree implementation for Myth metaserver.

A self-balancing binary search tree implementation that maintains balance
by coloring nodes red and black according to certain rules.

This implementation provides:
- O(log n) search, insert, delete operations
- Generic key and value types
- Validation of tree properties
...
class RedBlackTree(Generic[K, V]):
    ...
    def insert(self, key: K, data: V) -> Node[K, V]:
        ...
        if not parent:
            self.root = node
        elif self.comp_func(node.key, parent.key) < 0:
            parent.left = node
        else:
            parent.right = node
    ...
    def remove(self, node: Node[K, V]) -> None:
        ...
        # If removed node was black, fix properties
